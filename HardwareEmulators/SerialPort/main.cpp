...
struct UartBinaryUartCallbacks : public BinaryUartCallbacks
{
	UartBinaryUartCallbacks() { }
	virtual ~UartBinaryUartCallbacks() { }
	
	//Malformed/corrupted packet handler:
	virtual void InvalidPacket(const uint8_t* Buffer, const size_t& BufferLen)
	{ 
		printf("\nUartBinaryUartCallbacks: Invalid packet; bytes: (:");
		for(size_t i = 0; i < BufferLen; i++)
		{
			//Cheaply delineate fields for the human eye:
			//~ if ( (2 == i) || (4 == i) || (8 == i) || (10 == i) || (BufferLen - 3 == i) || (BufferLen - 2 == i) ) { printf("_"); }			
			printf("%.2X:", Buffer[i]);
		}
		printf(")\n");
	}
	...
	{ 
		printf("\nUartBinaryUartCallbacks: BufferOverflow(%zu)!\n", BufferLen);
	}
